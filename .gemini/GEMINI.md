...
3. **빠른 피드백 및 토큰 최적화 (Fail-Fast Thinking)**:
   문제 해결 방법이 불확실하여 4분 이상 길게 생각하게 될 경우(불필요한 토큰 사용 방지), 즉시 생각을 멈추고 현재 이해한 방향이 맞는지 사용자에게 명시적으로 재질문하여 방향성을 확인하십시오.
4. **수술적 수정(Surgical Update)의 정의**:
   효율성(Context usage)을 핑계로 코딩 표준(Standards)을 타협하지 마십시오. 단순히 코드 한 줄을 고치는 것이 아니라, 해당 변경에 필요한 **필수 import 추가 및 관련 스타일 준수**를 포함한 "완성된 단위"의 수정을 의미합니다.
5. **코딩 표준(standards.md) 엄격 준수**:
   작업 전 반드시 `standards.md`를 재검토하십시오. 특히 **FQN(전체 패키지 경로) 사용은 절대 금지**이며, 단 한 번의 사용이라도 반드시 import 문을 추가하고 클래스명만 사용해야 합니다. (예: `java.util.Date` → `import java.util.Date;` 후 `Date` 사용)