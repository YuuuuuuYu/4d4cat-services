...
class TechBlogRssCollectorTest {
  ...
  @Autowired private TechBlogDataScheduler dataScheduler;
  ...
  @MockitoBean private PixabayMusicCollector pixabayMusicCollector;
  ...
  void collectFeeds_shouldSavePosts() throws Exception {
    ...
    when(response.body())
        .thenReturn(new ByteArrayInputStream(mockRss.getBytes(StandardCharsets.UTF_8)));
        ...
        status -> {
          var savedPost =
              postRepository.findAll().stream()
                  .filter(
                      p -> p.getCompany().getSlug().equals(TechBlogFixtures.DEFAULT_COMPANY_SLUG))
                  .findFirst()
                  .orElseThrow();

          assertThat(savedPost.getTitle()).isEqualTo("우아한 고객경험을 설계하다.");
          assertThat(savedPost.getUrl())
              .isEqualTo("https://techblog.woowahan.com/self-pos-586169f788c7");
  ...
  void collectFeeds_withNaverAtomFeed_shouldUseUpdatedDate() throws Exception {
    ...
    when(response.statusCode()).thenReturn(200);
    when(response.body())
        .thenReturn(new ByteArrayInputStream(mockAtom.getBytes(StandardCharsets.UTF_8)));
