...
public class PixabayTestFixtures {
  ...
  public static void setupRestTemplateToReturnSingleVideo(RestTemplate restTemplate, int id) {
    ...
    PixabayResponse<PixabayVideoResult> response =
        new PixabayResponse<>(total, totalHits, List.of(videoResult));
  ...
  public static void setupRestTemplateToReturnSingleMusic(RestTemplate restTemplate, int id) {
    ...
    CustomPixabayMusicResponse<PixabayMusicResult> response =
        new CustomPixabayMusicResponse<>(query, page, total, List.of(musicResult));
  ...
  public static void setDataStorageToReturnAllResult(int videoCount, int musicCount) {
    ...
    List<PixabayVideoResult> videoList =
        IntStream.range(1, videoCount + 1)
            .mapToObj(PixabayTestFixtures::createDefaultVideoResult)
            .toList();
    List<PixabayMusicResult> musicList =
        IntStream.range(1, musicCount + 1)
  ...
  public static CustomPixabayMusicResponse createMusicResponse(
      List<PixabayMusicResult> musicResults) {
    return new CustomPixabayMusicResponse(
        "query", "1", String.valueOf(musicResults.size()), musicResults);
  }

  public static PixabayResponse createVideoResponse(List<PixabayVideoResult> videoResults) {
    return new PixabayResponse(
        String.valueOf(videoResults.size()), String.valueOf(videoResults.size()), videoResults);
  }
}
