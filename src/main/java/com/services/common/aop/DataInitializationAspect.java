...
public class DataInitializationAspect {
    ...
    private void sendWebhookNotification(String serviceName,
                                         String title,
                                         String description,
                                         int color) {
    ...
    private void sendErrorWebhook(String serviceName, ErrorCode errorCode, Exception exception) {
        String errorDescriptionFromCode = messageSource.getMessage(
                errorCode.getMessageKey(),
                new Object[]{exception.getMessage()},
                DEFAULT_LOCALE
        );

        String errorTitle = messageSource.getMessage("discord.init.failure.title", null, DEFAULT_LOCALE);
        String finalDescription = String.format("❌ 에러 코드: %s\n❌ 상세 내용: %s\n\n[MessageSource]: %s",
                errorCode.getCode(),
                exception.getMessage(),
                errorDescriptionFromCode
        );
    ...
    public Object logDataInitialization(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            String successTitle = messageSource.getMessage("discord.init.success.title", null, DEFAULT_LOCALE);
            String successDescription = messageSource.getMessage("discord.init.success.description",
                    new Object[]{serviceName, duration.toSeconds()}, DEFAULT_LOCALE);
