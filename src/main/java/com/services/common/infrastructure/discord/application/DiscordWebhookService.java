        ...
        return webClient.post()
                ...
                .onStatus(
                        ...
                        response -> response.bodyToMono(String.class)
                                .flatMap(body -> {
                                    log.error("Failed to send Discord webhook. Status: {}, Response: {}", response.statusCode(), body);
                                    return Mono.error(new DiscordWebhookException(ErrorCode.DISCORD_WEBHOOK_FAILED));
                                })
                )
